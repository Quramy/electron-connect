...
ProcessManager.prototype.spawn = function (args, spawnOpt) {
  ...
  if (this.electronProc.pid) electronPids.push(this.electronProc.pid);
...
ProcessManager.prototype.killProcess = function (cb) {
  if(this.electronProc) {
    ...
    var pid = this.electronProc.pid;
    var timeout = 3000;
    setTimeout(function() {
      kill(pid, 'SIGKILL', function() {
        for (var i = 0; i < electronPids.length; i++) {
          if (electronPids[i] === pid) {
            electronPids.splice(i, 1);
          }
        }
      });
    }, timeout);
...
function cleanUpAndExit() {
  if (electronPids && electronPids.length > 0) {
    for (var i = 0; i < electronPids; i++) {
      if (typeof electronPids[i] !== 'undefined') {
        console.log(
          '[' + new Date().toISOString() + '] [electron-connect] [server]',
          'killing electron process: ' + electronPids[i]
        );
        require('child_process').exec('kill ' + electronPids[i]);
      }
    }
  }
